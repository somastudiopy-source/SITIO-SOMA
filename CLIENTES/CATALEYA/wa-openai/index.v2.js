...
diff --git a/CLIENTES/CATALEYA/wa-openai/index.js b/CLIENTES/CATALEYA/wa-openai/index.js
index bbd1d6feb191ff1c5fb19683f723ad12b3d625a3..dccbd307e261384f9ba70bf251c9a6499883336b 100644
--- a/CLIENTES/CATALEYA/wa-openai/index.js
+++ b/CLIENTES/CATALEYA/wa-openai/index.js
@@ -16332,165 +16332,230 @@ app.get("/health", (req, res) => {
   res.json({
     ok: true,
     timeAR: nowARString(),
     tmpDir: getTmpDir(),
...
@@ -19321,50 +19386,53 @@ Si quiere, le paso información de cualquiera de esos 😊`;
...
+    }
 ...
 if (ENABLE_APPOINTMENT_TEMPLATES) {
   setInterval(() => {
diff --git a/CLIENTES/CATALEYA/wa-openai/index.v2.js b/CLIENTES/CATALEYA/wa-openai/index.v2.js
new file mode 100644
index 0000000000000000000000000000000000000000..ad46ea0d4a34bbf63e3b8bb1bb0c743a3a7d15a5
--- /dev/null
+++ b/CLIENTES/CATALEYA/wa-openai/index.v2.js
@@ -0,0 +1,1219 @@
...
+const COURSE_SHEET_NAME = "CURSOS";
+const COURSES_AUTOREPLY_ENABLED = String(process.env.V2_COURSES_AUTOREPLY || "").toLowerCase() === "true";
+
+if (!DB_URL) {
...
+let coursesCatalogCache = { ts: 0, rows: [] };
+
+function getCoursesSpreadsheetId() {
+  return process.env.TURNOS_SHEET_ID || "";
+}
+
+function normalizeCourseRow(row = {}) {
+  const name = String(row.nombre || row.curso || row["nombre curso"] || row["curso"] || "").trim();
+  if (!name) return null;
+  return {
+    name,
+    category: String(row.categoria || row.rubro || "").trim(),
+    modality: String(row.modalidad || "").trim(),
+    duration: String(row.duracion || row.duración || "").trim(),
+    price: String(row.precio || row.valor || "").trim(),
+    description: String(row.descripcion || row.descripción || "").trim(),
+  };
+}
+
+function courseLabel(course, idx) {
+  const details = [course.category, course.price].filter(Boolean).join(" · ");
+  return `${idx + 1}) ${course.name}${details ? ` — ${details}` : ""}`;
+}
+
+async function loadCoursesCatalogFromSheets() {
+  const spreadsheetId = getCoursesSpreadsheetId();
+  if (!spreadsheetId) throw new Error("Falta TURNOS_SHEET_ID para leer catálogo de cursos");
+  const now = Date.now();
+  if (coursesCatalogCache.rows.length && now - coursesCatalogCache.ts < 90 * 1000) return coursesCatalogCache.rows;
+  const saFile = process.env.GOOGLE_SA_FILE || "";
+  if (!saFile) throw new Error("Falta GOOGLE_SA_FILE para leer Google Sheets");
+  const auth = new google.auth.GoogleAuth({
+    keyFile: saFile,
+    scopes: ["https://www.googleapis.com/auth/spreadsheets.readonly"],
+  });
+  const sheets = google.sheets({ version: "v4", auth });
+  const result = await sheets.spreadsheets.values.get({ spreadsheetId, range: `${COURSE_SHEET_NAME}!A:Z` });
+  const rows = Array.isArray(result?.data?.values) ? result.data.values : [];
+  if (!rows.length) return [];
+  const headers = rows[0].map((h) => String(h || "").trim().toLowerCase());
+  const normalized = rows.slice(1).map((r) => {
+    const obj = {};
+    headers.forEach((h, i) => { obj[h] = r[i]; });
+    return normalizeCourseRow(obj);
+  }).filter(Boolean);
+  coursesCatalogCache = { ts: now, rows: normalized };
+  console.log("📚 COURSES_CATALOG_LOADED", JSON.stringify({ sheet: COURSE_SHEET_NAME, count: normalized.length }));
+  return normalized;
+}
+
...
+  if (!t) return null;
+  const byIndex = t.match(/\b(\d{1,2})\b/);
+  if (byIndex) {
+    const idx = Number(byIndex[1]) - 1;
+    if (idx >= 0 && idx < catalog.length) return catalog[idx];
+  }
+  const exact = catalog.find((c) => t === String(c.name || "").toLowerCase());
+  if (exact) return exact;
+  const includes = catalog.filter((c) => String(c.name || "").toLowerCase().includes(t) || t.includes(String(c.name || "").toLowerCase()));
+  if (includes.length === 1) return includes[0];
+  return includes.length > 1 ? { ambiguous: true, options: includes.slice(0, 5) } : null;
+}
+
+async function upsertCourseEnrollmentDraft({ waId, waPhone, selectedCourse }) {
+  await db.query(
+    `INSERT INTO course_enrollment_drafts (wa_id, wa_phone, course_name, course_category, flow_step, last_intent, updated_at)
+     VALUES ($1,$2,$3,$4,'course_selected','course_enrollment',NOW())
...
+       updated_at = NOW()`,
+    [String(waId || ""), String(waPhone || ""), String(selectedCourse?.name || ""), String(selectedCourse?.category || "")]
+  );
+}
+
...
+
+          if (!catalog.length) {
+            console.log("📚 COURSES_FLOW_TRANSITION", JSON.stringify({ wa_id: waId, stage: "catalog_empty" }));
+            continue;
+          }
+
...
+            if (COURSES_AUTOREPLY_ENABLED) {
+              await sendWhatsAppText(phone, listMsg);
+            } else {
+              console.log("📚 COURSES_REPLY_SKIPPED", JSON.stringify({ wa_id: waId, reason: "V2_COURSES_AUTOREPLY disabled", preview: listMsg.slice(0, 180) }));
+            }
+            continue;
+          }
+
+          const selection = selectCourseFromText(textForCourse, catalog);
+          if (!selection) {
+            if (existingCourseCtx) {
+              const ask = "No llegué a identificar el curso. ¿Me decís el nombre exacto o el número de la lista?";
+              console.log("📚 COURSES_FLOW_TRANSITION", JSON.stringify({ wa_id: waId, stage: "ask_clarification" }));
+              if (COURSES_AUTOREPLY_ENABLED) {
+                await sendWhatsAppText(phone, ask);
+              } else {
+                console.log("📚 COURSES_REPLY_SKIPPED", JSON.stringify({ wa_id: waId, reason: "V2_COURSES_AUTOREPLY disabled", preview: ask }));
+              }
+            }
+            continue;
+          }
...
+            const ask = `Tengo más de una opción parecida:\n${selection.options.slice(0, 5).map((c, idx) => courseLabel(c, idx)).join("\n")}\n\n¿Con cuál te quedás?`;
+            console.log("📚 COURSES_FLOW_TRANSITION", JSON.stringify({ wa_id: waId, stage: "selection_ambiguous", option_count: selection.options.length }));
+            if (COURSES_AUTOREPLY_ENABLED) {
+              await sendWhatsAppText(phone, ask);
+            } else {
+              console.log("📚 COURSES_REPLY_SKIPPED", JSON.stringify({ wa_id: waId, reason: "V2_COURSES_AUTOREPLY disabled", preview: ask.slice(0, 180) }));
+            }
+            continue;
+          }
+
+          setLastCourseContext(waId, { stage: "course_selected", selectedCourse: selection, catalogSnapshot: catalog.slice(0, 20) });
+          await upsertCourseEnrollmentDraft({ waId, waPhone: phone, selectedCourse: selection });
+          console.log("📚 COURSES_FLOW_TRANSITION", JSON.stringify({
+            wa_id: waId,
+            stage: "draft_updated",
+            course_name: selection.name,
+            course_category: selection.category || "",
+          }));
+          const confirmMsg = `Perfecto, te anoto en preinscripción para *${selection.name}*.\nEn la próxima etapa pedimos tus datos (nombre, DNI y teléfono).`;
+          if (COURSES_AUTOREPLY_ENABLED) {
+            await sendWhatsAppText(phone, confirmMsg);
+          } else {
+            console.log("📚 COURSES_REPLY_SKIPPED", JSON.stringify({ wa_id: waId, reason: "V2_COURSES_AUTOREPLY disabled", preview: confirmMsg.slice(0, 180) }));
+          }
+        }
+      }
+    }
